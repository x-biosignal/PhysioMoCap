...
- target_rate:
  ...
  Target sampling rate in Hz. If NULL (default), the highest sampling
  rate among the inputs is used.
...
A list of PhysioExperiment objects, all with the same sampling rate and
the same number of rows (time points).
...
for resampling individual PhysioExperiment objects,
[`resampleVector()`](https://x-biosignal.github.io/PhysioMoCap/reference/resampleVector.md)
for resampling raw numeric vectors.
